...
## 1. Tipos avanzados (type, interface, record...)
...
`Function` y `function` no es lo mismo, la palabra reservada `function` se usa para declarar métodos/funciones, pero la palabra `Function` se usa para tipar variables para que devuelvan o almacenen estrictamente métodos/funciones.